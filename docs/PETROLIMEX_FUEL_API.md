...
- **Logic**:
  1. Gọi `syncAndSave(force=true)` — cào + lưu DB
  2. Nếu hôm nay chưa có row → sao chép giá mới nhất từ ngày trước
  3. Ghim (`is_published = true`) giá của ngày hôm nay, bỏ ghim các ngày khác
...
- `effective_at`: thời điểm giá có hiệu lực chính xác (trích từ thông cáo)
